...
con.query(`SELECT * FROM bans WHERE id = '${msg.mentions.users.first().id}'`, (err, rows) => {
    ...
    if(rows.length < 1 ) {
    sql = `INSERT INTO bans (id, banned) VALUES ('${msg.mentions.users.first().id}', '1')`
    } else {
        let bans = rows[0].bans;
        sql = `UPDATE bans SET banned = 1 WHERE id = '${msg.mentions.users.first().id}'`;
    }
    con.query(sql);
})

            msg.channel.send('MWHAHAHHAHAHHAHAH, done i guess?') 
        } else {
