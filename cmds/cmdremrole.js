    ...
    if (args.length <= 1)
    {
        ...
        if (msg.member.roles.some(r=>config.perm.includes(r.name)) || msg.author.id == config.owner ) {
            try {
                ...
                console.log(`remrole has been run by ${msg.author.username}#${msg.author.discriminator} (ID: "${msg.author.id}")` )
                let role = msg.guild.roles.find('name', args[1]);
                let member = msg.mentions.members.first();
                member.removeRole(role).catch(error => {msg.channel.send(`Error: ${error}`)});
                msg.channel.send("Done.")
                
            } catch (error) {
                console.log(error)
                msg.channel.send(error)
                Embeds.error(msg.channel, `Usage of this command: !remrole @member Name_of_the_role
